...
echo "  1) Personal  — provider's standard API endpoint"
echo "  2) Company   — Azure OpenAI (paste the full endpoint URL)"
echo ""
...
case "$ENDPOINT_CHOICE" in
  ...
  2)
    ...
    read -rp "Full Azure endpoint URL: " AZURE_FULL_URL
    [ -z "$AZURE_FULL_URL" ] && error "Endpoint URL cannot be empty."
    AZURE_API_BASE=$(echo "$AZURE_FULL_URL" | grep -oE 'https://[^/]+')
    AZURE_DEPLOYMENT=$(echo "$AZURE_FULL_URL" | grep -oE '/deployments/[^/?]+' | sed 's|/deployments/||')
    AZURE_API_VERSION=$(echo "$AZURE_FULL_URL" | grep -oE 'api-version=[^/&]+' | sed 's|api-version=||')
    [ -z "$AZURE_API_BASE" ]   && error "Could not parse Azure base URL — check the endpoint URL."
    [ -z "$AZURE_DEPLOYMENT" ] && error "Could not parse deployment name — check the endpoint URL."
    [ -z "$AZURE_API_VERSION" ] && AZURE_API_VERSION="2025-01-01-preview"
    info "Parsed → base: ${AZURE_API_BASE}  deployment: ${AZURE_DEPLOYMENT}  version: ${AZURE_API_VERSION}"
    read -rsp "API key: " API_KEY; echo
    ;;
  *) error "Invalid choice." ;;
esac
...
AGENT_MODEL=azure/${AZURE_DEPLOYMENT}
HUB_MODEL=azure/${AZURE_DEPLOYMENT}
